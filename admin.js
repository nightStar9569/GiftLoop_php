...
async function uploadAdminFile(formData) {
  ...
  const makeUrl = (path) => {
    try {
      ...
      return "/GiftLoop/api/admin/upload";
    }
  };
  const resp = await fetch(makeUrl("/admin/upload"), {
    method: "POST",
    body: formData,
    credentials: "include",
  });
...
function initUserForm() {
  ...
  const avatarUrlEl = document.getElementById("userAvatarUrl");
  const avatarFileEl = document.getElementById("userAvatarFile");
  if (avatarFileEl) {
    avatarFileEl.addEventListener("change", async () => {
      const file = avatarFileEl.files && avatarFileEl.files[0];
      if (!file) return;
      try {
        showNotification("アバターをアップロード中...", "info");
        const fd = new FormData();
        fd.append("file", file);
        const url = await uploadAdminFile(fd);
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!email || !password) {
      showNotification("メールとパスワードを入力してください。", "error");
      return;
    }
    try {
      await giftApi.adminUserCreate({
        email,
        password,
        firstName,
        lastName,
...
// Update navbar avatar after login/status refresh
(async function updateNavAvatar(){
  try {
    const me = await (window.giftApi?.me?.());
    const url = me?.user?.avatarUrl || me?.user?.avatar_url;
    const holder = document.getElementById('navAvatar');
    if (holder && url) {
      holder.innerHTML = `<img src="${url}" alt="avatar" style="width:100%;height:100%;object-fit:cover" />`;
    }
  } catch(_) {}
})();
    ...
    (async () => {
      try {
        ...
        let items = (res.items || []).filter((g) => {
          ...
          const matchesStatus =
            activeStatus === "all" || g.status === activeStatus;
        ...
        switch (sortEl?.value) {
          case "price_desc":
            items.sort((a, b) => ((a.price || 0) < (b.price || 0) ? 1 : -1));
            break;
          case "price_asc":
    ...
    items.forEach((g) => {
      ...
      row.innerHTML = `
        <div class="admin-row-main">
          ${
            g.image_url
              ? `<div class="thumb" style="width:56px;height:56px;border-radius:10px;overflow:hidden;flex:0 0 auto;background:#f3f4f6;display:flex;align-items:center;justify-content:center"><img src="${
                  g.image_url
                }" alt="${
                  g.name || ""
                }" style="width:100%;height:100%;object-fit:cover" /></div>`
              : `<div class="icon" style="width:56px; height:56px; display:flex; align-items:center; justify-content:center; border-radius:10px; background:#f3f4f6; color:${
                  g.isSuper ? "#ffd700" : "#e91e63"
                };"><i class="fas ${
                  g.isSuper ? "fa-crown" : "fa-gift"
                }"></i></div>`
          }
          <div class="meta">
            ...
            <div class="sub">¥${(
              g.price || 0
            ).toLocaleString()} ・ ${getCategoryText(
        g.category
      )} ・ 状態: ${getStatusText(g.status)}</div>
          </div>
        </div>
      ...
      row
        ...
        .addEventListener("click", async () => {
          ...
          try {
            await giftApi.adminGiftDelete(g.id);
      ...
      row
        ...
        .addEventListener("click", async () => openGiftEditModal(g));
...
function initCorporatePanel() {
  ...
  async function render() {
    ...
    try {
      ...
      if (!items.length) {
        listEl.innerHTML =
          '<div class="empty-card">会社ギフトがありません</div>';
        return;
      }
      items.forEach((c) => {
        const row = document.createElement("div");
          ...
          <div class="admin-row-main">
            <div style="display:flex; gap:12px; align-items:center">
              ...
              <div>
                ...
                <div class="sub">カテゴリ: ${getCategoryText(
                  c.category
                )} ・ 期間: ${c.start_at} 〜 ${c.end_at}</div>
              </div>
            </div>
          </div>
          <div class="admin-row-actions">
        ...
        row
          ...
          .addEventListener("click", async () => {
            ...
            try {
              await giftApi.adminCorporateDelete(c.id);
        ...
        row
          .querySelector('[data-action="edit"]')
          .addEventListener("click", () => openCorpEditModal(c));
  ...
  if (form && !form._bound) {
    form.addEventListener("submit", async (e) => {
      ...
      const payload = {
        ...
        totalSupply: getVal("corpEditTotalSupply")
          ? Number(getVal("corpEditTotalSupply"))
          : null,
...
function initUsersPanel() {
  ...
  async function render() {
    ...
    users.forEach((u) => {
      ...
      tr.innerHTML = `
        <td><div style="display:flex;align-items:center;gap:10px"><span style="width:28px;height:28px;border-radius:50%;overflow:hidden;display:inline-flex;align-items:center;justify-content:center;background:#f3f4f6">${u.avatar_url ? `<img src="${u.avatar_url}" alt="avatar" style="width:100%;height:100%;object-fit:cover" />` : `<i class="fas fa-user"></i>`}</span><span>${(u.first_name || "") + " " + (u.last_name || "")}</span></div></td>
        <td>${u.email}</td>
        <td>${u.birth_date || "-"}</td>
        <td>${u.gifts_sent || 0}</td>
        <td>${u.gifts_received || 0}</td>
        <td>${getLevelText(u.membership_level)}</td>
        <td>${u.is_admin ? "管理者" : "-"}</td>
        <td>
          <button class="btn-primary" data-action="edit" aria-label="編集">編集</button>
          <button class="btn-primary" data-action="delete" aria-label="削除" style="margin-left:.5rem;">削除</button>
        </td>
      `;
      ...
      tr.querySelector('[data-action="delete"]').addEventListener(
        ...
        async () => {
          ...
          try {
            await giftApi.adminUserDelete(u.id);
...
function openUserEditModal(u) {
  ...
  setVal("userEditId", u.id);
  setVal("userEditFirst", u.first_name || "");
  setVal("userEditLast", u.last_name || "");
  setVal("userEditBirth", u.birth_date || "");
  setVal("userEditAvatar", u.avatar_url || "");
  const level = u.membership_level || "basic";
  const sel = document.getElementById("userEditLevel");
  ...
  if (form && !form._bound) {
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      const id = getVal("userEditId");
      const payload = {
        firstName: getVal("userEditFirst") || null,
        lastName: getVal("userEditLast") || null,
        birthDate: getVal("userEditBirth") || null,
        membershipLevel:
          document.getElementById("userEditLevel")?.value || "basic",
        avatarUrl: getVal("userEditAvatar") || null,
      };
      try {
        await giftApi.adminUserUpdate(id, payload);
  ...
  const fileEl = document.getElementById("userEditAvatarFile");
  if (fileEl && !fileEl._bound) {
    fileEl.addEventListener("change", async () => {
      const file = fileEl.files && fileEl.files[0];
      if (!file) return;
  ...
  const cancel = document.getElementById("userEditCancel");
  if (cancel && !cancel._bound) {
    cancel.addEventListener("click", closeUserEditModal);
    cancel._bound = true;
  }
  modal.style.display = "flex";
}
...
function getLevelText(level) {
  ...
  return map[String(level || "").toLowerCase()] || "ベーシック";
}
